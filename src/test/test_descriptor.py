...
class DescriptorTests(unittest.TestCase):
    ...
    def test_features_and_depth(self):
        ...
        cases = [
            ...
            (f'tr({k1})',
             0, MS_IS_DESCRIPTOR, 2, 1),
            # Taptree branches contribute to descriptor depth.
            (f'tr({k1},{{pk({k1}),{{pk({k1}),pk({k1})}}}})',
             0, MS_IS_DESCRIPTOR|MS_IS_X_ONLY|MS_IS_TAPSCRIPT, 5, 4),
        ]
        ...
        # Tapleaves must be top-level B expressions. K, V, and W expressions
        # are valid only as subexpressions and must be rejected as leaf roots.
        for non_b_leaf in (f'pk_k({k1})', f'v:pk({k1})', f'a:pk({k1})'):
            d = c_void_p()
            ret = wally_descriptor_parse(f'tr({k1},{non_b_leaf})', None,
                                         NETWORK_NONE, 0, d)
            self.assertEqual(ret, WALLY_EINVAL, non_b_leaf)

        # A nested expression must consume its complete substring.
        trailing_garbage = (
            f'tr({k1},pk({k1})garbage)',
            f'tr({k1},{{pk({k1}),pk({k1})garbage}})',
            f'wsh(pk({k1})garbage)',
            f'sh(wsh(pk({k1})garbage))',
            f'wsh(and_v(v:pk({k1}),pk({k1})garbage))',
        )
        for invalid_descriptor in trailing_garbage:
            d = c_void_p()
            ret = wally_descriptor_parse(invalid_descriptor, None,
                                         NETWORK_NONE, 0, d)
            self.assertEqual(ret, WALLY_EINVAL, invalid_descriptor)
