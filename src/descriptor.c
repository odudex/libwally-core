...
#define PROP_CHANGE(keep, set) *properties &= (TYPE_MASK | keep); *properties |= set
        ...
        case 'd':
            ...
            if (ctx->features & WALLY_MS_IS_TAPSCRIPT)
                *properties |= PROP_U; /* tapscript: d: gains u property */
            break;
        case 'v':
            PROP_REQUIRE(TYPE_B);
...
{
    if (descriptor->features & WALLY_MS_IS_TAPROOT)
        return descriptor->top_node->child->next;
    return NULL;
}
    ...
    unsigned char *bytes_out, size_t len, size_t *written)
{
    ...
    taptree = tr_get_tree(descriptor); /* Checked by _len() above */
    ...
    unsigned char *bytes_out, size_t len)
{
    ...
    ret = compute_taptree_hash(&ctx, tr_get_tree(descriptor), bytes_out, len);
    wally_free(ctx.path_buff);
    return ret;
}
