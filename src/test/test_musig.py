...
class MuSig2Tests(unittest.TestCase):
    ...
    def test_synthetic_xpub_construction(self):
        ...
        ret = wally_musig_pubkey_agg(pub_keys_flat, len(pub_keys_flat),
                                     None, 0, byref(cache))
        self.assertEqual(WALLY_OK, ret)
        agg_pk, _ = make_cbuffer('00' * EC_PUBLIC_KEY_LEN)
        self.assertEqual(WALLY_OK,
                         wally_musig_pubkey_get(cache, agg_pk, EC_PUBLIC_KEY_LEN))
        wally_musig_keyagg_cache_free(cache)

        xpub = POINTER(ext_key)()
        ret = wally_musig_pubkey_to_xpub(agg_pk, EC_PUBLIC_KEY_LEN,
    ...
    def test_synthetic_xpub_unhardened_derivation(self):
        ...
        xpub = POINTER(ext_key)()
        wally_musig_pubkey_to_xpub(agg_pk, EC_PUBLIC_KEY_LEN,
                                   BIP32_VER_MAIN_PUBLIC, byref(xpub))
    ...
    def test_synthetic_xpub_invalid_args(self):
        ...
        agg_pk_buf, _ = make_cbuffer('02' + '01' * EC_XONLY_PUBLIC_KEY_LEN)
        xpub = POINTER(ext_key)()

        # NULL pubkey
        self.assertEqual(WALLY_EINVAL,
                         wally_musig_pubkey_to_xpub(None, EC_PUBLIC_KEY_LEN,
                                                    BIP32_VER_MAIN_PUBLIC, byref(xpub)))
        # Wrong length (x-only keys are not accepted: the parity is required)
        self.assertEqual(WALLY_EINVAL,
                         wally_musig_pubkey_to_xpub(agg_pk_buf, EC_XONLY_PUBLIC_KEY_LEN,
                                                    BIP32_VER_MAIN_PUBLIC, byref(xpub)))
        # Not a compressed pubkey prefix
        bad_prefix, _ = make_cbuffer('04' + '01' * EC_XONLY_PUBLIC_KEY_LEN)
        self.assertEqual(WALLY_EINVAL,
                         wally_musig_pubkey_to_xpub(bad_prefix, EC_PUBLIC_KEY_LEN,
                                                    BIP32_VER_MAIN_PUBLIC, byref(xpub)))
        # Invalid version (private key version)
        self.assertEqual(WALLY_EINVAL,
                         wally_musig_pubkey_to_xpub(agg_pk_buf, EC_PUBLIC_KEY_LEN,
    ...
    def test_agg_then_derive(self):
        ...
        self.assertEqual(WALLY_OK, wally_musig_pubkey_agg(sorted_keys_flat, len(sorted_keys_flat),
                                                          None, 0, byref(cache)))
        agg_pk, _ = make_cbuffer('00' * EC_PUBLIC_KEY_LEN)
        self.assertEqual(WALLY_OK, wally_musig_pubkey_get(cache, agg_pk, EC_PUBLIC_KEY_LEN))
        wally_musig_keyagg_cache_free(cache)
        synthetic_xpub = POINTER(ext_key)()
        self.assertEqual(WALLY_OK, wally_musig_pubkey_to_xpub(agg_pk, EC_PUBLIC_KEY_LEN,
    ...
    def test_psbt_musig2_add_nonce(self):
        ...
        # The input must spend the descriptor's scriptpubkey to be populated
        ...
        self.assertEqual(WALLY_OK,
                         wally_tx_output_init_alloc(200000, bytes(spk[:34]), 34, utxo))
        ...
        pidx = idx2 if (ret2 == WALLY_OK and idx2 > 0) else idx3

        # Fetch the two registered participant pubkeys for this aggregate key:
        # add_nonce only accepts pubkeys from the registered participant set
        parts_buf, _ = make_cbuffer('00' * (2 * EC_PUBLIC_KEY_LEN))
        ret, written = wally_map_get_item(byref(inp.musig2_pubkeys), pidx - 1,
                                          parts_buf, 2 * EC_PUBLIC_KEY_LEN)
        self.assertEqual((ret, written), (WALLY_OK, 2 * EC_PUBLIC_KEY_LEN))
        pk1 = bytes(parts_buf[:EC_PUBLIC_KEY_LEN])
        pk2 = bytes(parts_buf[EC_PUBLIC_KEY_LEN:2 * EC_PUBLIC_KEY_LEN])
        ...
        ret, count = wally_psbt_input_get_musig2_pubnonce_size(psbt.contents.inputs[0])
        ...
        ret, count2 = wally_psbt_input_get_musig2_pubnonce_size(psbt.contents.inputs[0])
    ...
    def test_psbt_musig2_finalize_input(self):
        ...
        # Fetch the real compressed aggregate key (the parity is not fixed)
        agg_pubkey_buf, _ = make_cbuffer('00' * EC_PUBLIC_KEY_LEN)
        self.assertEqual(WALLY_OK,
                         wally_musig_pubkey_get(cache, agg_pubkey_buf, EC_PUBLIC_KEY_LEN))
        ...
        ret, nonce_count = wally_psbt_input_get_musig2_pubnonce_size(psbt.contents.inputs[0])
        self.assertEqual(WALLY_OK, ret)
        self.assertEqual(0, nonce_count, 'musig2 pubnonces should be cleared after finalize')
        ret, sig_count = wally_psbt_input_get_musig2_partial_sig_size(psbt.contents.inputs[0])
        self.assertEqual(WALLY_OK, ret)
    ...
    def test_psbt_musig2_wrong_aggregate_key(self):
        ...
        # Fetch the real compressed aggregate key (the parity is not fixed)
        real_agg_buf, _ = make_cbuffer('00' * EC_PUBLIC_KEY_LEN)
        self.assertEqual(WALLY_OK,
                         wally_musig_pubkey_get(cache, real_agg_buf, EC_PUBLIC_KEY_LEN))

        # An invalid (non-curve-point) aggregate key must be rejected outright
        # by the participants map verifier when registering.
        invalid_agg = bytes([0x02]) + bytes([0x03] * EC_XONLY_PUBLIC_KEY_LEN)
        invalid_agg_buf, _ = make_cbuffer(invalid_agg.hex())

        # Build a wrong-but-valid aggregate key: aggregating in the reverse
        # order yields a different (but valid) aggregate key.
        wrong_xonly, _ = make_cbuffer('00' * EC_XONLY_PUBLIC_KEY_LEN)
        wrong_cache = c_void_p()
        self.assertEqual(WALLY_OK, wally_musig_pubkey_agg(pk2 + pk1, len(pub_keys_flat),
                                                          wrong_xonly, EC_XONLY_PUBLIC_KEY_LEN,
                                                          wrong_cache))
        wrong_agg_full, _ = make_cbuffer('00' * EC_PUBLIC_KEY_LEN)
        self.assertEqual(WALLY_OK,
                         wally_musig_pubkey_get(wrong_cache, wrong_agg_full, EC_PUBLIC_KEY_LEN))
        wally_musig_keyagg_cache_free(wrong_cache.value)
        wrong_agg_buf = wrong_agg_full
        ...
        participants_flat = bytes(pk1) + bytes(pk2)
        # An invalid aggregate key is rejected when registering participants.
        self.assertEqual(WALLY_EINVAL,
                         wally_psbt_input_add_musig2_participant_pubkeys(
                             psbt.contents.inputs,
                             invalid_agg_buf, EC_PUBLIC_KEY_LEN,
                             participants_flat, len(participants_flat)))

        # Register participant pubkeys under the WRONG (but valid) aggregate key.
        self.assertEqual(WALLY_OK,
                         wally_psbt_input_add_musig2_participant_pubkeys(
                             psbt.contents.inputs,
    ...
    @unittest.skipUnless(wally_musig_pubkey_agg, 'MuSig2 module not enabled')
    def test_psbt_musig2_finalize_rejects_corrupt_partial_sig(self):
        """finalize_input must fail on a corrupt partial sig and keep the evidence"""
        pk1 = derive_pubkey(SECKEY1)
        pk2 = derive_pubkey(SECKEY2)
        pub_keys_flat = pk1 + pk2
        ...
        cache = c_void_p()
        self.assertEqual(WALLY_OK, wally_musig_pubkey_agg(pub_keys_flat, len(pub_keys_flat),
                                                          agg_pk_xonly, EC_XONLY_PUBLIC_KEY_LEN,
                                                          cache))
        agg_pubkey_buf, _ = make_cbuffer('00' * EC_PUBLIC_KEY_LEN)
        self.assertEqual(WALLY_OK,
                         wally_musig_pubkey_get(cache, agg_pubkey_buf, EC_PUBLIC_KEY_LEN))
        ...
        # Round 1: nonces, Round 2: both participants sign
        secrand1, _ = make_cbuffer('f1' * 32)
        secrand2, _ = make_cbuffer('f2' * 32)
        ...
        # Corrupt participant 1's stored partial sig (flip a byte, keeping
        # a valid length/scalar) by replacing the map entry.
        inp = psbt.contents.inputs[0]
        ret, idx = wally_psbt_input_find_musig2_partial_sig(
            inp, pk1, EC_PUBLIC_KEY_LEN, agg_pubkey_buf, EC_PUBLIC_KEY_LEN, None, 0)
        self.assertEqual(WALLY_OK, ret)
        self.assertGreater(idx, 0)
        sig_val, _ = make_cbuffer('00' * MUSIG_PARTIAL_SIG_LEN)
        ret, written = wally_map_get_item(byref(inp.musig2_partial_sigs), idx - 1,
                                          sig_val, MUSIG_PARTIAL_SIG_LEN)
        self.assertEqual((ret, written), (WALLY_OK, MUSIG_PARTIAL_SIG_LEN))
        corrupted = bytearray(sig_val)
        corrupted[8] ^= 0x01
        self.assertEqual(WALLY_OK,
                         wally_psbt_input_add_musig2_partial_sig(
                             inp, pk1, EC_PUBLIC_KEY_LEN,
                             agg_pubkey_buf, EC_PUBLIC_KEY_LEN, None, 0,
                             bytes(corrupted), MUSIG_PARTIAL_SIG_LEN))

        # Finalize must fail: the corrupt partial sig does not verify
        ret = wally_psbt_musig2_finalize_input(
            psbt, 0, agg_pubkey_buf, EC_PUBLIC_KEY_LEN, None, 0, cache.value, 0)
        self.assertNotEqual(WALLY_OK, ret,
                            'finalize must fail with a corrupt partial sig')

        # The evidence (pubnonces and partial sigs) must be preserved
        ret, nonce_count = wally_psbt_input_get_musig2_pubnonce_size(inp)
        self.assertEqual((ret, nonce_count), (WALLY_OK, 2),
                         'pubnonces must be preserved when finalize fails')
        ret, sig_count = wally_psbt_input_get_musig2_partial_sig_size(inp)
        self.assertEqual((ret, sig_count), (WALLY_OK, 2),
                         'partial sigs must be preserved when finalize fails')

        # No TAP_KEY_SIG may have been stored
        sig_buf, _ = make_cbuffer('00' * EC_SIGNATURE_LEN)
        ret, sig_written = wally_psbt_get_input_taproot_signature(psbt, 0, sig_buf,
                                                                  EC_SIGNATURE_LEN)
        self.assertTrue(ret != WALLY_OK or sig_written == 0,
                        'TAP_KEY_SIG must not be stored when finalize fails')
